#!/usr/bin/env python3
"""
Layer plots + significance heatmaps (Welch one-sided) + head score delta heatmaps
+ head norm heatmaps + score-vs-norm delta scatter plots (pooled, language-mean,
and per-language).
...
CLI:
  ...
  --max_head_layer N               (default: 9)
    -> head-based plots only use rows with layer <= max_head_layer

Important:
- The wide dataframe is FILTERED to only (control + modes) immediately after loading.
- The head dataframe is FILTERED to only (control + modes), and then to head rows
  with layer <= max_head_layer.
...
def load_head_parquet(input_parquet: str) -> pd.DataFrame:
    ...
    if "head_norm" in df.columns:
        df["head_norm"] = pd.to_numeric(df["head_norm"], errors="coerce")
...
def _pivot_layer_head_mean(
    df_slice: pd.DataFrame, value_col: str = "score"
) -> pd.DataFrame:
    return (
        df_slice.groupby(["layer", "head"], as_index=False)[value_col]
        .mean()
        .pivot(index="layer", columns="head", values=value_col)
        .sort_index(axis=0)
        .sort_index(axis=1)
    )
...
def _plot_delta_with_consistency_alpha(
    mean_mat: np.ndarray,
    ...
    vmin: float,
    vmax: float,
    cbar_label: str = "mean(score) delta",
):
    apply_plot_style()
    fig, ax = plt.subplots(figsize=(14.0, 7.5))
    ...
    for a, lab in [(0.25, "low consistency"), (0.6, "medium"), (1.0, "high")]:
        ax.scatter([], [], c="k", alpha=a, label=lab)
    ax.legend(frameon=True, loc="upper right", fontsize=9, title="alpha = consistency")

    save_fig(fig, out_path, fmt=plot_format)


def _compute_language_agnostic_tables(
    ...
    g = (
        df_model.groupby(["lang", "mode", "layer", "head"], as_index=False)[value_col]
        .mean()
    ...
    a = (
        g[g["mode"] == mode_a]
        .rename(columns={"mean_value": "a_value"})
        .drop(columns=["mode"])
    )
    b = (
        g[g["mode"] == mode_b]
        .rename(columns={"mean_value": "b_value"})
        .drop(columns=["mode"])
    )

    m = a.merge(b, on=["lang", "layer", "head"], how="inner")
    m["delta"] = m["a_value"] - m["b_value"]

    per_lang_delta_long = m[["lang", "layer", "head", "delta"]].copy()

    agg = (
        per_lang_delta_long.groupby(["layer", "head"])["delta"]
        .agg(n_lang="count", mean_delta="mean", std_delta="std")
        .reset_index()
    )
    eps = 1e-9
    agg["consistency_raw"] = np.abs(agg["mean_delta"]) / (
        agg["std_delta"].fillna(0.0) + eps
    )
    return per_lang_delta_long, agg


def _consistency_to_alpha(consistency_raw: np.ndarray) -> np.ndarray:
    x = np.asarray(consistency_raw, dtype=float)
    x = x[np.isfinite(x)]
    if x.size == 0:
        return np.zeros_like(consistency_raw, dtype=float)

    p95 = float(np.percentile(x, 95))
    denom = p95 if np.isfinite(p95) and p95 > 0 else 1.0
    a = np.asarray(consistency_raw, dtype=float) / denom
    return np.clip(a, 0.0, 1.0)


def _compute_pooled_head_deltas(
    df_model: pd.DataFrame,
    mode_a: str,
    mode_b: str,
    value_col: str,
    ...
    g = (
        df_model.groupby(["mode", "layer", "head"], as_index=False)[value_col]
        .mean()
        .rename(columns={value_col: "mean_value"})
    )
    a = (
    ...
    m = a.merge(b, on=["layer", "head"], how="inner")
    m["delta"] = m["a_value"] - m["b_value"]
    return m[["layer", "head", "delta"]].copy()


def _compute_per_language_head_deltas(
    df_model: pd.DataFrame,
    mode_a: str,
    mode_b: str,
    value_col: str,
    ...
    m = a.merge(b, on=["lang", "layer", "head"], how="inner")
    m["delta"] = m["a_value"] - m["b_value"]
    return m[["lang", "layer", "head", "delta"]].copy()


def _plot_score_vs_norm_scatter(
    ...
    d = merged_df.copy()
    d = d[np.isfinite(d["score_delta"]) & np.isfinite(d["norm_delta"])].copy()
    if d.empty:
        return
    ...
    ax.scatter(
        d["norm_delta"].to_numpy(),
        d["score_delta"].to_numpy(),
        s=28,
        alpha=0.75,
    )
    ...
    ax.set_title(title)
    ax.set_xlabel("head norm delta")
    ax.set_ylabel("score delta")

    d["interest"] = np.sqrt(d["score_delta"] ** 2 + d["norm_delta"] ** 2)
    top = d.sort_values("interest", ascending=False).head(label_top_k)

    for _, row in top.iterrows():
        ax.annotate(
            f"L{int(row['layer'])}H{int(row['head'])}",
            (row["norm_delta"], row["score_delta"]),
            xytext=(3, 3),
...
def _plot_head_metric_family(
    ...
    piv_pooled = {
        mode: _pivot_layer_head_mean(df_m[df_m["mode"] == mode], value_col=metric_col)
        for mode in needed
    }
    ...
    for lang, df_ml in df_m.groupby("lang", sort=True):
        piv_by_lang[lang] = {
            mode: _pivot_layer_head_mean(
                df_ml[df_ml["mode"] == mode], value_col=metric_col
            )
            for mode in needed
        }

    model_name = str(df_m["model"].iloc[0])

    for name, mode_a, mode_b in delta_pairs:
        pa = piv_pooled.get(mode_a, pd.DataFrame())
        pb = piv_pooled.get(mode_b, pd.DataFrame())
        if not pa.empty and not pb.empty:
            ...
            lim = _robust_symmetric_lim(diff.to_numpy().ravel(), pct=99.0)

            out_tsv = os.path.join(
                stats_dir, f"{metric_slug}_delta_{safe_filename(name)}_alllangs.tsv"
            )
            diff.reset_index().to_csv(out_tsv, sep="\t", index=False)

            out_path = os.path.join(
                out_dir,
                f"heatmap_{metric_slug}_delta_{safe_filename(name)}_alllangs.{plot_format}",
            )
            _plot_heatmap(
                mat=diff.to_numpy(),
                x_labels=[int(h) for h in heads],
                y_labels=[int(l) for l in layers],
                title=f"{model_name} | Δ{metric_label} ({mode_a} − {mode_b}) | all languages",
                out_path=out_path,
                plot_format=plot_format,
                cmap="RdBu_r",
                vmin=-lim,
                vmax=lim,
                cbar_label=f"mean({metric_col}) delta",
            )

        bylang_dir = os.path.join(out_dir, "by_language", safe_filename(name))
        bylang_stats_dir = os.path.join(stats_dir, "by_language", safe_filename(name))
        os.makedirs(bylang_dir, exist_ok=True)
        os.makedirs(bylang_stats_dir, exist_ok=True)

        for lang, piv_lang in piv_by_lang.items():
            pa_l = piv_lang.get(mode_a, pd.DataFrame())
            pb_l = piv_lang.get(mode_b, pd.DataFrame())
            if pa_l.empty or pb_l.empty:
                continue

            layers_l = sorted(set(pa_l.index) | set(pb_l.index))
            heads_l = sorted(set(pa_l.columns) | set(pb_l.columns))

            da_l = pa_l.reindex(index=layers_l, columns=heads_l)
            db_l = pb_l.reindex(index=layers_l, columns=heads_l)
            diff_l = da_l - db_l

            lim_l = _robust_symmetric_lim(diff_l.to_numpy().ravel(), pct=99.0)

            out_tsv_l = os.path.join(
                bylang_stats_dir, f"{metric_slug}_delta_{safe_filename(lang)}.tsv"
            )
            diff_l.reset_index().to_csv(out_tsv_l, sep="\t", index=False)

            out_path_l = os.path.join(
                bylang_dir,
                f"heatmap_{metric_slug}_delta_{safe_filename(lang)}.{plot_format}",
            )
            _plot_heatmap(
                mat=diff_l.to_numpy(),
                x_labels=[int(h) for h in heads_l],
                y_labels=[int(l) for l in layers_l],
                title=f"{model_name} | {lang} | Δ{metric_label} ({mode_a} − {mode_b})",
                out_path=out_path_l,
                plot_format=plot_format,
                cmap="RdBu_r",
                vmin=-lim_l,
        ...
        per_lang_delta_long, agg = _compute_language_agnostic_tables(
            df_m, mode_a, mode_b, value_col=metric_col
        )

        agg_out = os.path.join(
            stats_dir, f"{metric_slug}_agnostic_{safe_filename(name)}.tsv"
        )
        agg.to_csv(agg_out, sep="\t", index=False)

        agg2 = agg.copy()
        agg2["abs_mean_delta"] = np.abs(agg2["mean_delta"])
        agg2["interest"] = agg2["abs_mean_delta"] * agg2["consistency_raw"]
        top = agg2.sort_values("interest", ascending=False).head(50)
        top_out = os.path.join(
            stats_dir, f"{metric_slug}_agnostic_top50_{safe_filename(name)}.tsv"
        )
        top.to_csv(top_out, sep="\t", index=False)

        if not agg.empty:
            layers = sorted(agg["layer"].unique().tolist())
            heads = sorted(agg["head"].unique().tolist())

            mean_mat = (
                agg.pivot(index="layer", columns="head", values="mean_delta")
                .reindex(index=layers, columns=heads)
                .to_numpy()
            )
            std_mat = (
                agg.pivot(index="layer", columns="head", values="std_delta")
                .reindex(index=layers, columns=heads)
                .to_numpy()
            )
            cons_raw_mat = (
                agg.pivot(index="layer", columns="head", values="consistency_raw")
                .reindex(index=layers, columns=heads)
                .to_numpy()
            )

            lim = _robust_symmetric_lim(mean_mat.ravel(), pct=99.0)

            ag_dir = os.path.join(out_dir, "language_agnostic")
            ...
            _plot_heatmap(
                ...
                x_labels=[int(h) for h in heads],
                y_labels=[int(l) for l in layers],
                title=f"{model_name} | mean Δ{metric_label} ({mode_a} − {mode_b}) across languages",
                out_path=os.path.join(
                    ag_dir,
                    f"heatmap_{metric_slug}_mean_delta_{safe_filename(name)}.{plot_format}",
                ),
                plot_format=plot_format,
                cmap="RdBu_r",
                vmin=-lim,
                vmax=lim,
                cbar_label="mean delta across langs",
            )

            std_vals = std_mat[np.isfinite(std_mat)]
            vmax_std = float(np.percentile(std_vals, 99)) if std_vals.size else 1.0
            vmax_std = max(vmax_std, 1e-6)

            _plot_heatmap(
                mat=np.nan_to_num(std_mat, nan=vmax_std),
                x_labels=[int(h) for h in heads],
                y_labels=[int(l) for l in layers],
                title=f"{model_name} | std(Δ{metric_label}) across languages",
                out_path=os.path.join(
                    ag_dir,
                    f"heatmap_{metric_slug}_std_delta_{safe_filename(name)}.{plot_format}",
                ),
                plot_format=plot_format,
                cmap="viridis",
                vmin=0.0,
                vmax=vmax_std,
                cbar_label="std of delta across langs",
            )

            alpha_mat = _consistency_to_alpha(cons_raw_mat)
            _plot_delta_with_consistency_alpha(
                mean_mat=mean_mat,
                alpha_mat=alpha_mat,
                x_labels=[int(h) for h in heads],
                y_labels=[int(l) for l in layers],
                title=(
                    f"{model_name} | Δ{metric_label} ({mode_a} − {mode_b}) | "
                    f"color=mean delta, alpha=language-consistency"
                ),
                out_path=os.path.join(
                    ag_dir,
                    f"heatmap_{metric_slug}_delta_with_consistency_{safe_filename(name)}.{plot_format}",
                ),
                plot_format=plot_format,
                vmin=-lim,
                vmax=lim,
...
def _plot_score_norm_delta_scatters(
    df_m: pd.DataFrame,
    out_dir: str,
    stats_dir: str,
    plot_format: str,
    ...
    if "head_norm" not in df_m.columns:
        return

    dfn = df_m.dropna(subset=["head_norm"]).copy()
    if dfn.empty:
        return

    model_name = str(df_m["model"].iloc[0])
    scatter_dir = os.path.join(out_dir, "score_vs_norm_scatter")
    scatter_stats_dir = os.path.join(stats_dir, "score_vs_norm_scatter")
    os.makedirs(scatter_dir, exist_ok=True)
    os.makedirs(scatter_stats_dir, exist_ok=True)

    for name, mode_a, mode_b in delta_pairs:
        # pooled across languages
        score_pooled = _compute_pooled_head_deltas(
            df_m, mode_a, mode_b, value_col="score"
        )
        norm_pooled = _compute_pooled_head_deltas(
            dfn, mode_a, mode_b, value_col="head_norm"
        )
        pooled = score_pooled.merge(
            norm_pooled,
            on=["layer", "head"],
            how="inner",
            suffixes=("_score", "_norm"),
        ).rename(columns={"delta_score": "score_delta", "delta_norm": "norm_delta"})

        pooled_out = os.path.join(
            scatter_stats_dir, f"pooled_{safe_filename(name)}.tsv"
        )
        pooled.to_csv(pooled_out, sep="\t", index=False)

        _plot_score_vs_norm_scatter(
            merged_df=pooled,
            title=f"{model_name} | pooled heads | score delta vs norm delta | {mode_a} − {mode_b}",
            out_path=os.path.join(
                scatter_dir,
                f"score_vs_norm_delta_pooled_{safe_filename(name)}.{plot_format}",
            ),
            plot_format=plot_format,
            label_top_k=20,
        )
        ...
        # language-mean
        score_lang = _compute_per_language_head_deltas(
            df_m, mode_a, mode_b, value_col="score"
        )
        norm_lang = _compute_per_language_head_deltas(
            dfn, mode_a, mode_b, value_col="head_norm"
        )
        lang_merged = score_lang.merge(
            norm_lang,
            on=["lang", "layer", "head"],
            how="inner",
            suffixes=("_score", "_norm"),
        ...
        lang_mean = lang_merged.groupby(["layer", "head"], as_index=False)[
            ["score_delta", "norm_delta"]
        ].mean()

        lang_mean_out = os.path.join(
            scatter_stats_dir, f"langmean_{safe_filename(name)}.tsv"
        )
        lang_mean.to_csv(lang_mean_out, sep="\t", index=False)

        _plot_score_vs_norm_scatter(
            merged_df=lang_mean,
            title=f"{model_name} | language-mean heads | score delta vs norm delta | {mode_a} − {mode_b}",
            out_path=os.path.join(
                scatter_dir,
                f"score_vs_norm_delta_langmean_{safe_filename(name)}.{plot_format}",
            ),
            plot_format=plot_format,
            label_top_k=20,
        )
        ...
        # per-language
        per_lang_dir = os.path.join(scatter_dir, "by_language", safe_filename(name))
        per_lang_stats_dir = os.path.join(
            scatter_stats_dir, "by_language", safe_filename(name)
        )
        os.makedirs(per_lang_dir, exist_ok=True)
        os.makedirs(per_lang_stats_dir, exist_ok=True)

        for lang, g_lang in lang_merged.groupby("lang", sort=True):
            lang_df = g_lang[
                ["lang", "layer", "head", "score_delta", "norm_delta"]
            ].copy()
            lang_out = os.path.join(per_lang_stats_dir, f"{safe_filename(lang)}.tsv")
            lang_df.to_csv(lang_out, sep="\t", index=False)

            _plot_score_vs_norm_scatter(
                merged_df=lang_df,
                title=f"{model_name} | {lang} | score delta vs norm delta | {mode_a} − {mode_b}",
                out_path=os.path.join(
                    per_lang_dir,
                    f"score_vs_norm_delta_{safe_filename(lang)}.{plot_format}",
                ),
                plot_format=plot_format,
                label_top_k=20,
            )
...
def head_delta_plots(
    ...
    df = df[df["layer"] <= int(max_head_layer)].copy()

    delta_pairs = [(f"{m}_minus_{control}", m, control) for m in modes]

    for model, df_m in df.groupby("model", sort=True):
        model_root = os.path.join(output_dir, safe_filename(model))
        os.makedirs(model_root, exist_ok=True)

        score_out_dir = os.path.join(model_root, "heads_deltas")
        score_stats_dir = os.path.join(score_out_dir, "stats")
        os.makedirs(score_out_dir, exist_ok=True)
        os.makedirs(score_stats_dir, exist_ok=True)

        _plot_head_metric_family(
            df_m=df_m,
            metric_col="score",
            metric_label="score",
            metric_slug="score",
            out_dir=score_out_dir,
            stats_dir=score_stats_dir,
            plot_format=plot_format,
            needed=needed,
            delta_pairs=delta_pairs,
        )

        if "head_norm" in df_m.columns:
            norm_df = df_m.dropna(subset=["head_norm"]).copy()
            if not norm_df.empty:
                norm_out_dir = os.path.join(model_root, "heads_norms")
                norm_stats_dir = os.path.join(norm_out_dir, "stats")
                os.makedirs(norm_out_dir, exist_ok=True)
                os.makedirs(norm_stats_dir, exist_ok=True)

                _plot_head_metric_family(
                    df_m=norm_df,
                    metric_col="head_norm",
                    metric_label="head norm",
                    metric_slug="head_norm",
                ...
                _plot_score_norm_delta_scatters(
                    df_m=df_m,
                    out_dir=model_root,
                    stats_dir=score_stats_dir,
                    plot_format=plot_format,
...
def main():
    p = argparse.ArgumentParser(
        description="Plot layer metrics + Welch one-sided significance heatmaps + head score/norm heatmaps + score-vs-norm scatter plots."
    )
    p.add_argument(
        "--input_tsv",
        required=True,
    ...
    p.add_argument(
        ...
        required=False,
        default=None,
        help="Long head parquet with lang, model, mode, layer, head, score, optionally head_norm.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=9,
        help="Maximum head layer to include in head heatmaps / norm plots / scatter plots (inclusive).",
    )
    ...
    p.add_argument(
        "--disable_head_deltas",
        action="store_true",
        help="Skip head delta/norm heatmaps and score-vs-norm scatter plots.",
    )
    ...
    if (args.heads_tsv is not None) and (not args.disable_head_deltas):
        head_delta_plots(
            ...
            control=args.control,
            modes=modes,
            max_head_layer=args.max_head_layer,
        )
