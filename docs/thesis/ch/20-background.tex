...
\textbf{Tokenization} is the process of segmentation and representation of our strings for use in corpora and machine learning systems \citep{jm3}. For example, how would we segment the English phrase `it's'? One alternative is to split it, \texttt{[it, 's]}. Another would be to remove the apostrophe, \texttt{[it, s]}. We could consider \texttt{[it, is]} as well. The list goes on. Those are the types of questions and decisions tokenization concerns. Traditionally, tokenization was seen as mapping input to dictionaries \citep{mielke2021}.
...
\item \textbf{Pre-tokenization:} For the BPE algorithm we need to know what we are compressing \textit{towards}, i.e.\ some sort of idea of what a \textit{token} can be vs. what it can't be. In GPT2, for example, pre-tokenization involves not only splitting on spaces and punctuation, but also replacing all bytes of the given string with a corresponding representation. Since there are only 256 unique bytes, this allows an efficient reduction of the vocabulary size while still retaining no unknown tokens during inference. It does mean, however, that some characters --- especially those with different script than English --- are represented by several byte-level tokens during the pre-tokenization stage. For example, the Chinese string `可以跟你借一個火嗎？' is represented by the equivalent string `åı¯ä»¥è·Łä½łåĢŁä¸ĢåĢĭçģ«åĹİï¼Ł' after pre-tokenization using byte-level BPE.


\item \textbf{Merge:} We define a desired vocabulary size, and perform merges based on the paired frequencies of our current vocabulary applied to our training set. For a a byte-level BPE tokenizer our base vocabulary at the start of training will be a few special tokens (like \texttt{<|endoftext|>} $\rightarrow$ 50256) along with all possible bytes (1 - 256). For a character-level BPE tokenizer it will be the set of unique characters in our training data. For each merge we pick the most frequent pair as our next merge candidate, and then repeat until we reach our desired vocabulary size.

\end{enumerate}

During inference time BPE-style tokenizers are by design greedy, and pick the best compression of a text. \citet{liu2025b} introduced SuperBPE, which uses non-greedy sampling during training to overcome the greedy design, with the hope to achieve more robust and performant language models, though in pre-trained models this is rare.
